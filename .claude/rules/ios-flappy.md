...
- **No image or audio assets in v1, except the app icon.** Everything in the scene is drawn procedurally. The icon must be a PNG (asset-catalog requirement); it is generated by `studio/art/make_app_icon.py`, never hand-painted. Adding any other binary asset requires a `studio-art` note explaining why a shape can't do it.
